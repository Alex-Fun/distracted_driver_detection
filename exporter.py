...
# 添加output节点并讲outputs字典元素value添加到指定集合名称下
...
# 将训练输出文件复制到当前指定的输出路径（即是model_path）
...
# 将数据输入input_tensor（即input_type对应的形式）通过模型model作用的结果outputs返回
